...
const SEED_PATIENTS = [
  {
    ...
    avatar: '🧑',
    prescriptions: [
      { id: 101, title: 'Cardiology Rx – Apollo', doctor: 'Dr. Sunita Rao', spec: 'Cardiologist, Apollo Hospital', date: '2026-01-18', status: 'VERIFIED', meds: 3, notes: '' },
      { id: 102, title: 'General Checkup', doctor: 'Dr. Ramesh Gupta', spec: 'General Physician', date: '2026-02-03', status: 'PENDING', meds: null, notes: 'Under review by pharmacist' },
    ],
  },
  {
    id: 2,
    ...
    avatar: '👩',
    prescriptions: [
      { id: 201, title: 'Dermatology Rx – Skin Clinic', doctor: 'Dr. Kavitha Nair', spec: 'Dermatologist, Fortis Hospital', date: '2026-01-09', status: 'REJECTED', meds: null, notes: 'Prescription expired or stamp missing' },
    ],
  },
  {
    id: 3,
    ...
    avatar: '👴',
    prescriptions: [
      { id: 301, title: 'Orthopaedic Rx – Nanavati', doctor: 'Dr. Anil Joshi', spec: 'Orthopaedic Surgeon, Nanavati Hospital', date: '2026-01-22', status: 'VERIFIED', meds: 2, notes: '' },
      { id: 302, title: 'Neurology Follow-up', doctor: 'Dr. Priya Krishnan', spec: 'Neurologist, Hinduja Hospital', date: '2026-02-07', status: 'VERIFIED', meds: 1, notes: 'Repeat prescription approved' },
    ],
  },
];
...
const RELATION_AVATARS = { Self:'🧑', Spouse:'👩', Father:'👴', Mother:'👵', Son:'👦', Daughter:'👧', Sibling:'🧒', Other:'👤' };

// ─── Helpers ──────────────────────────────────────────────────────────────────
function statusMeta(status) {
  if (status === 'VERIFIED') return { bg: styles.bgGreen,  txt: styles.textGreen,  badge: styles.badgeGreen,  icon: '✓' };
...
function Toast({ msg, type, onClose }) {
  ...
  return (
    <div className={`${styles.toast} ${styles['toast' + type]}`}>
      <span>{type === 'success' ? '✅' : type === 'warn' ? '⚠️' : 'ℹ️'} {msg}</span>
  ...
  const savePatient = () => {
    ...
    if (editPId) {
      setPatients(prev => prev.map(p => p.id === editPId
        ? { ...p, ...pForm, avatar: RELATION_AVATARS[pForm.relation] || '👤' }
        : p));
      showToast(`${pForm.name} updated!`);
    } else {
      const np = { id: Date.now(), ...pForm, avatar: RELATION_AVATARS[pForm.relation] || '👤', prescriptions: [] };
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          {[['/', '🏠', 'Home'], ['/prescriptions','📁','Vault'], ['/orders','🛍️','Orders'], ['/profile','👤','Profile'], ['/settings','⚙️','Settings']].map(([href, icon, label]) => (
            <Link key={href} href={href} className={`${styles.navItem} ${pathname === href ? styles.navActive : ''}`}>
      ...
      <main className={styles.main}>
        ...
        <header className={styles.topHeader}>
          <div className={styles.headerLeft}>
            <span className={styles.headerShieldIcon}>🛡️</span>
                ...
                >
                  ...
                  {selectedId === p.id ? (
                    <div className={styles.patientActions}>
                      ...
                      <button className={styles.pActionBtn} onClick={e => { e.stopPropagation(); confirmDeletePatient(p); }} title="Remove">🗑️</button>
                    </div>
                  ) : (
                    ...
                    <div key={rx.id} className={styles.card} data-reveal="true" data-delay={i * 80}>
                      ...
                      <div className={styles.cardBody}>
                        ...
                        <div className={styles.metaInfo}>
                          <p><span>👨‍⚕️</span>{rx.doctor}</p>
                          <p><span>📅</span>Issued: {rx.date}</p>
                          {rx.meds  && <p><span>💊</span>{rx.meds} medicine{rx.meds > 1 ? 's' : ''} prescribed</p>}
                          {rx.notes && <p className={rx.status === 'REJECTED' ? styles.errorText : ''}><span>📄</span>{rx.notes}</p>}
                        </div>
                      </div>
                      <div className={styles.cardFooter}>
                        {rx.status === 'VERIFIED' && (
                          <Link href="/medicines" className={`btn btn-primary ${styles.actionBtn}`}>Order Now</Link>
                        )}
                        {rx.status === 'PENDING' && (
                        ...
                        <button className={styles.iconActionBtn} title="View details" onClick={() => openViewRx(rx)}>👁️</button>
                        <button className={`${styles.iconActionBtn} ${styles.iconDelBtn}`} title="Delete" onClick={() => confirmDeleteRx(rx)}>🗑️</button>
                      </div>
                    </div>
                  );
                })}
      ...
      {modal === 'addPatient' && (
        <Modal title={editPId ? 'Edit Patient' : 'Add New Patient'} onClose={closeModal}>
          ...
          <div className={styles.previewAvatar}>
            <span>{RELATION_AVATARS[pForm.relation] || '👤'}</span>
            ...
            <div className={styles.viewRxContent}>
              ...
              {viewRx.status === 'VERIFIED' && (
                <div className={styles.viewRxAction}>
                  <Link href="/medicines" className="btn btn-primary">💊 Order Medicines</Link>
                </div>
              )}
