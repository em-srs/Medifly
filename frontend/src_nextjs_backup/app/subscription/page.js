...
// ─── Static sample data ───────────────────────────────────────────────────────
const INITIAL_MEDS = [
  { id: 1, name: 'Atorvastatin 20mg',  qty: 30, unit: 'Tablets', freq: 30, icon: '💊', color: 'iconPill',  nextRefill: '2026-02-14' },
  { id: 2, name: 'Metformin 500mg ER', qty: 60, unit: 'Tablets', freq: 30, icon: '🏥', color: 'iconKit',   nextRefill: '2026-02-14' },
];

const HISTORY = [
  { id: 'REF-2026-004', date: 'Jan 14, 2026', meds: ['Atorvastatin 20mg × 30', 'Metformin 500mg × 60'], amount: '₹340', status: 'Delivered' },
  { id: 'REF-2026-003', date: 'Dec 14, 2025', meds: ['Atorvastatin 20mg × 30', 'Metformin 500mg × 60'], amount: '₹340', status: 'Delivered' },
  { id: 'REF-2026-002', date: 'Nov 14, 2025', meds: ['Atorvastatin 20mg × 30'],                         amount: '₹189', status: 'Delivered' },
  { id: 'REF-2026-001', date: 'Oct 14, 2025', meds: ['Atorvastatin 20mg × 30', 'Metformin 500mg × 60'], amount: '₹340', status: 'Skipped'   },
];
...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div className={`${styles.toast} ${styles['toast' + type]}`}>
      <span>{type === 'Success' ? '✅' : type === 'Warning' ? '⚠️' : 'ℹ️'} {message}</span>
...
export default function SubscriptionPage() {
  ...
  const medsRef    = useScrollReveal();
  const plansRef   = useScrollReveal();
  const historyRef = useScrollReveal();
  const plansPageRef = useScrollReveal();
    ...
    <div className={styles.pageWrap}>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.pageHeader} ref={headerRef}>
          <div data-reveal="true" data-delay="0">
        ...
        {activeTab === 'active' && (
          <div className={styles.grid}>
            ...
            <div className={styles.leftCol}>
              {/* Member card */}
              <div className={`${styles.memberCard} ${isPaused ? styles.memberCardPaused : ''}`} data-reveal="true" data-delay="0">
                  ...
                  <div className={styles.memberInfo}>
                    ...
                    <div>
                      ...
                      <p className={styles.memberDate}>
                        Next Refill: <strong>Nov 12, 2024</strong>
                      </p>
              ...
              <div className={styles.infoCardsGrid}>
                ...
                <div className={styles.infoCard} data-reveal="right" data-delay="0">
                  <div className={`${styles.infoIcon} ${styles.iconBell}`}>🔔</div>
                  <div className={styles.infoContent}>
                ...
                <div className={styles.plansContainer}>
                  {PLANS.map((plan, i) => (
                    <div key={plan.id} className={`${styles.planCard} ${plan.highlight ? styles.planHighlight : ''} ${currentPlan === plan.id ? styles.planCurrent : ''}`} data-reveal="scale" data-delay={i * 100}>
                      ...
                      >
                        {currentPlan === plan.id ? '✓ Current Plan' : plan.id === 'yearly' ? 'Switch to Yearly' : `Select ${plan.name}`}
                      </button>
                    </div>
                  ))}
                </div>
            ...
            <div className={styles.plansPageGrid}>
              {PLANS.map((plan, i) => (
                <div key={plan.id} className={`${styles.planPageCard} ${plan.highlight ? styles.planPageHighlight : ''} ${currentPlan === plan.id ? styles.planPageCurrent : ''}`}
                  ...
                  {plan.highlight && <div className={styles.planPageBadge}>⭐ BEST VALUE</div>}
                  <h3>{plan.name}</h3>
                  ...
                  <button
                    ...
                    disabled={currentPlan === plan.id}
                  >
                    {currentPlan === plan.id ? '✓ Your Current Plan' : `Select ${plan.name}`}
                  </button>
                </div>
              ))}
            </div>
      ...
      {modal === 'manage' && (
        <Modal title="Manage Your Plan" onClose={closeModal}>
          <div className={styles.manageGrid}>
            ...
            <div className={styles.manageDetail}><span>Next Billing</span><strong>Nov 12, 2024</strong></div>
            ...
            {isPaused ? (
              ...
              <>
                ...
                <div className={styles.pauseInfo}>
                  <span>🔔</span> You can resume anytime from this page.
                </div>
              </>
            )}
          </div>
        ...
        <Modal title={`Reschedule: ${medToEdit.name}`} onClose={closeModal}>
          ...
          <div className={styles.calendarTips}>
            ...
            <div className={styles.quickDates}>
              {['2024-11-20','2024-11-25','2024-12-01','2024-12-12'].map(d => (
      ...
      {modal === 'selectPlan' && planPreview && (
        <Modal title="Confirm Plan Change" onClose={closeModal}>
          ...
          {planPreview.save && (
            <div className={styles.planSaveNote}>🎉 You'll save <strong>{planPreview.save}</strong> with this plan!</div>
          )}
          <ul className={styles.modalFeatureList}>
            {planPreview.features.map(f => <li key={f}>✓ {f}</li>)}
          </ul>
          <div className={styles.modalFooter}>
      ...
      {modal === 'schedule' && (
        <Modal title="Modify Delivery Schedule" onClose={closeModal}>
          ...
          <div className={styles.schedulePreview}>
            📅 Next deliveries: <strong>{scheduleDay} Nov</strong> · <strong>{scheduleDay} Dec</strong> · <strong>{scheduleDay} Jan</strong>
          </div>
          <div className={styles.modalFooter}>
      ...
      {modal === 'skipRefill' && (
        <Modal title="Skip Next Refill" onClose={closeModal}>
          <div className={styles.pauseContent}>
            ...
            <p>Your next scheduled refill on <strong>Nov 12, 2024</strong> will be skipped. Your subscription will continue from <strong>December</strong> onwards.</p>
            <div className={styles.pauseInfo}>
              <span>💰</span> You will not be charged for the skipped cycle.
            </div>
          </div>
          <div className={styles.modalFooter}>
