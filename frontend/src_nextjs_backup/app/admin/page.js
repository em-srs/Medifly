        ...
        <div className="tabs">
          {['overview', 'pharmacies', 'riders', 'orders', 'disputes'].map(tab => (
            <button key={tab} className={`tab ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab)}>
              {tab === 'overview' ? '📊 Overview' : tab === 'pharmacies' ? '🏥 Pharmacies' : tab === 'riders' ? '🚴 Riders' : tab === 'orders' ? '📦 Orders' : '⚠️ Disputes'}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'overview' && (
            <>
              <div className={styles.statsGrid}>
                <div className={styles.statCard}><span>👥</span><div><strong>{adminStats.totalUsers.toLocaleString()}</strong><small>Total Users</small></div></div>
                <div className={styles.statCard}><span>📦</span><div><strong>{adminStats.totalOrders.toLocaleString()}</strong><small>Total Orders</small></div></div>
                <div className={styles.statCard}><span>💰</span><div><strong>₹{(adminStats.totalRevenue / 100000).toFixed(1)}L</strong><small>Total Revenue</small></div></div>
                <div className={styles.statCard}><span>🏥</span><div><strong>{adminStats.activePharmacies}</strong><small>Active Pharmacies</small></div></div>
                <div className={styles.statCard}><span>🚴</span><div><strong>{adminStats.activeRiders}</strong><small>Active Riders</small></div></div>
                <div className={styles.statCard}><span>⚡</span><div><strong>{adminStats.avgDeliveryTime}</strong><small>Avg Delivery</small></div></div>
                <div className={styles.statCard}><span>📊</span><div><strong>{adminStats.slaCompliance}%</strong><small>SLA Compliance</small></div></div>
                <div className={styles.statCard}><span>⭐</span><div><strong>{adminStats.customerSatisfaction}/5</strong><small>Satisfaction</small></div></div>
              </div>
                ...
                {sampleOrders.map(order => (
                  <div key={order.id} className={styles.liveOrder}>
                    ...
                    <div className={styles.liveOrderInfo}>
                      ...
                      {order.rider && <span>🚴 {order.rider}</span>}
