      ...
      <div className="container" ref={pageRef}>
        <div className={styles.header} data-reveal="true" data-delay="0">
          ...
          <div className={styles.headerStats}>
            ...
            <div className={styles.hStat}><strong>4.8 ⭐</strong><span>Rating</span></div>
          </div>
        </div>
        ...
        <div className="tabs">
          {['orders', 'prescriptions', 'stock', 'earnings'].map(tab => (
            <button key={tab} className={`tab ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab)}>
              {tab === 'orders' ? '📦 Orders' : tab === 'prescriptions' ? '📋 Prescriptions' : tab === 'stock' ? '📊 Stock' : '💰 Earnings'}
            </button>
          ))}
        </div>
        ...
        <div className={styles.content} ref={contentRef}>
                  ...
                  <div className={styles.orderFooter}>
                    ...
                    {!order.pharmacyAction && order.status === 'Processing' && (
                      <div className={styles.orderActions}>
                        <button className="btn btn-primary btn-sm" onClick={() => handleAction(order.id, 'accepted')}>✅ Accept</button>
          ...
          {activeTab === 'earnings' && (
            <div className={styles.earningsGrid}>
              <div className={styles.earningCard}><span>📦</span><div><strong>78</strong><small>Orders Today</small></div></div>
              <div className={styles.earningCard}><span>💰</span><div><strong>₹52,000</strong><small>Revenue Today</small></div></div>
              <div className={styles.earningCard}><span>📊</span><div><strong>₹3,82,000</strong><small>This Month</small></div></div>
              <div className={styles.earningCard}><span>⭐</span><div><strong>4.8</strong><small>Rating</small></div></div>
            </div>
          )}
        </div>
      </div>
