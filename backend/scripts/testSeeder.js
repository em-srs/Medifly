...
const testRun = async () => {
  try {
    ...
    console.log('Testing 100 rows insert from meds_dB_original.csv...');
    ...
    await new Promise((resolve, reject) => {
      fs.createReadStream(ORIGINAL_CSV_PATH)
        .pipe(csv())
        .on('data', (data) => {
          if (rows.length < 100) rows.push(data);
        })
        .on('end', resolve)
        .on('error', reject);
    });
    ...
    if (saltMap.size < uniqueSalts.length) {
      const fetchRes = await client.query('SELECT id, salt_name FROM salts WHERE salt_name = ANY($1)', [uniqueSalts]);
      fetchRes.rows.forEach(s => saltMap.set(s.salt_name, s.id));
    }
    ...
    console.log('✅ TEST SEED SUCCESSFUL FOR 100 ROWS!');
